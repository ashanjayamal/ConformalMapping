...
\begin{definition}
\protect\hypertarget{def:unnamed-chunk-29}{}\label{def:unnamed-chunk-29}We define the length of \(\gamma\) in the hyperbolic metric by
...
\begin{definition}
\protect\hypertarget{def:unnamed-chunk-30}{}\label{def:unnamed-chunk-30}The hyperbolic distance \(\rho(z_0, z_1)\) from \(z_0\) to \(z_1\) is defined as the infimum (greatest lower bound) of the hyperbolic lengths of all piecewise smooth curves in \(\mathbb{D}\) from \(z_0\) to \(z_1\).
\[\rho(z_0, z_1)
  =\inf_\gamma\left\{\text{hyperbolic length of } \gamma \right\}
  =\inf_\gamma\left\{2\int_{\gamma} \frac{|dz|}{1-|z|^2}\right\}\]
\end{definition}

\includegraphics{figures/Riemann_Mapping_Therom/fig1.png}
...
\includegraphics{figures/Riemann_Mapping_Therom/fig2.png}

\begin{theorem}
\protect\hypertarget{thm:unnamed-chunk-33}{}\label{thm:unnamed-chunk-33}For any two distinct points \(z_0, z_1\) in the open unit disk \(\mathbb{D}\), there exists a unique shortest curve in \(\mathbb{D}\) from \(z_0\) to \(z_1\) in the hyperbolic metric. Specifically, this curve corresponds to the arc of the circle passing through \(z_0\) and \(z_1\) that is orthogonal to the unit circle.
\end{theorem}

\begin{definition}
\protect\hypertarget{def:unnamed-chunk-34}{}\label{def:unnamed-chunk-34}The paths of shortest hyperbolic length between points are called \textbf{hyperbolic geodesics}.
\end{definition}
...
\includegraphics{figures/Riemann_Mapping_Therom/fig3.png}
...
\begin{lemma}
\protect\hypertarget{lem:unnamed-chunk-37}{}\label{lem:unnamed-chunk-37}Let \(D\) be a simply connected domain. Suppose \(a \notin D\), and let
...
\begin{theorem}[Riemann Mapping Theorem]
\protect\hypertarget{thm:unnamed-chunk-39}{}\label{thm:unnamed-chunk-39}Given any simply connected region \(\Omega\) which is not the whole plane, and a point \(z_0\) in \(\Omega\), there exists a unique analytic function \(f(z)\) in \(\Omega\), normalized by the conditions \(f(z_0) = 0\), \(f'(z_0) > 0\), such that \(f(z)\) defines a one-to-one mapping of \(\Omega\) onto the disk \(|w| < 1\).
\end{theorem}
