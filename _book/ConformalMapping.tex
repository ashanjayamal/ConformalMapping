...
\protect\hypertarget{def:unnamed-chunk-26}{}\label{def:unnamed-chunk-26}The Riemann sphere, also called the extended complex plane consist of the complex numbers \(\mathbb{C}\) together with \(\infty\). The set of extended complex numbers may be written as \(\mathbb{C} \cup \{\infty\}\).
\end{definition}
...
\protect\hypertarget{def:unnamed-chunk-30}{}\label{def:unnamed-chunk-30}A function \(f(z)\) is meromorphic on a domain \(D\) if \(f(z)\) is analytic on \(D\) except possibly at isolated singularities, each of which is a pole.
\end{definition}
...
\includegraphics{ConformalMapping_files/figure-latex/unnamed-chunk-45-1.pdf}
